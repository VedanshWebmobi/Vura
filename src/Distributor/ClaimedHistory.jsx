...
export default function ClaimedHistory({ searchText }) {
  ...
  const fetchOrderHistory = async (startDate, endDate) => {
    setloader(true);
    console.log(
      "Fetching order history on Claimed:",
    ...
    try {
      ...
      console.error("Your response:", newData);
      if (currentPage !== 1) {
  ...
  const handlePending = (item) => {
    navigation.navigate("ClaimedOrder", { item });
  };

  const renderItem = ({ item, index }) => (
    <PendingClaimCard item={item} onPress={() => handlePending(item)} isClaim />
  );
